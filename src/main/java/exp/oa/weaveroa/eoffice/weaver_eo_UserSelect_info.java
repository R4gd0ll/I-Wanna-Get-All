...
public class weaver_eo_UserSelect_info implements Exploitlnterface {
    @Override
    ...
    private Boolean att(String url, TextArea textArea) {
        try {
            ...
            Response get_res = HttpTools.get(url + "/UserSelect/" , head, "utf-8");
            if (get_res.getCode() == 200 && get_res.getText().contains("/UserSelect/top.php")) {
                Platform.runLater(() -> {
                    textArea.appendText("\n [ weaver_eo_UserSelect_info ]");
                    textArea.appendText("\n 人员页面: "+url + "/UserSelect");
                    textArea.appendText("\n");
                });
